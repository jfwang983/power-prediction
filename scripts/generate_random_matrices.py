...
code_dir = "templates/bareMetalC"
code_list = [code for code in os.listdir(code_dir) if re.match(r'^.*\.c$', code) and code != 'simple.c']

define_pattern = re.compile(r'#define\s+(\w+)\s+(\d+)')
sizes = {}

rand_range = {
    "elem_t": {"min": -128, "max": 127},
    "acc_t": {"min": -2147483648, "max": 2147483647}
}

for code in code_list:
    with open(f"{code_dir}/{code}", "r") as file:
    ...
    for line in lines:
        if not re.search(r"// Matrix [\w\s]+ row", line.strip()):
            ...
            if match_message:
                ...
                features = matrix_match.group(2).split(', ')
                element_type, matrix_dims = features[0], features[1:]
                ...
                if len(matrix_dims) == 2:
                    ...
                    modified_lines.append(f"\t{element_type} {matrix_name}[{matrix_rows}][{matrix_columns}] = {{ // Matrix {matrix_name} row\n")

                    for i in range(matrix_rows_nums):
                        row_line = "\t\t{"
                        for j in range(matrix_columns_nums):
                            rand_value = random.randint(rand_range[element_type]["min"], rand_range[element_type]["max"])
                    ...
                    modified_lines.append(f"\t{element_type} {matrix_name}[{matrix_dim_1}][{matrix_dim_2}][{matrix_dim_3}][{matrix_dim_4}] = {{ // Matrix {matrix_name} row\n")
    ...
    with open(f"{code_dir}/{code}", "w") as file:
