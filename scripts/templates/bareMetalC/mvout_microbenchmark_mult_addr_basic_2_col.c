...
#define AccData_rows 16
#define AccData_cols 64
#define C_rows 16
#define C_cols 2
...
int main() {
  ...
  // Matrix AccData, acc_t, AccData_rows, AccData_cols Setup
	...
	// Matrix AccData2, acc_t, AccData_rows, AccData_cols Setup

  acc_t C[C_rows][C_cols];
  ...
  gemmini_config_st(C_cols * sizeof(acc_t));
  ...
  for(int i = 0; i < ACC_ROWS; i += 128) {
    gemmini_extended_mvin(AccData, C_acc_base_addr + i, 64, DIM);
    gemmini_extended_mvin(AccData2, C_acc_base_addr + i + 64, 64, DIM);
  }
