            ...
            for (size_t row = 0; row < rows; row += NORM_STAT_IDS) {
              ...
              for (int cmd = 0; cmd < norm_cmds_size; cmd++) {
                for (size_t stat_id = 0; stat_id < stat_ids; stat_id++) {
                  ...
                  #ifdef STATIC_ANNOTATE
                  printf("gemmini_config_norm(0, 0, 0, 0, %u, 0, 0);\n", stat_id);
                  #endif
                  ...
                  for (size_t jj = 0; jj < J; jj += C_blocks) {
                    ...
                    #ifdef STATIC_ANNOTATE
                    printf("gemmini_extended_mvout(%p, 0x%x, %u, %u);\n", C_dram_addr, norm_C_sp_addr, cols, 1);
                    #endif
                  }
                }
              }
...
/*  // Combined loop. We want to generate spike outputs. 
